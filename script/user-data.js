  ...
  if (xhr.readyState === 4) {
    if (xhr.status === 200) {
      ...
      if (response.error) {
        ...
        document.getElementById("tanggal-pengguna").textContent =
          response.tanggal;
        document.getElementById("alamat-pengguna").textContent =
          response.alamat;
...
xhr.open("GET", "../src/get-user-data.php", true); // Sesuaikan URL dengan lokasi file PHP yang digunakan
xhr.send();
